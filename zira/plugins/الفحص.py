...
async def amireallyalive(event):
    ...
    ALIVE_TEXT = gvarstatus("ALIVE_TEXT") or "** بـوت  نينجا 𝗡𝗶𝗡𝗝𝗔  يعمـل .. بنجـاح ☑️ 𓆩 **"
...
**{Z_EMOJI} قنـاة السـورس :** [اضغـط هنـا](https://t.me/fv_av)"""
...
async def amireallyialive(event):
    ...
    reply_to_id = await reply_id(event)
    Z_EMOJI = gvarstatus("ALIVE_EMOJI") or "✥┊"
    zed_caption = "** بـوت  نينجا 𝗡𝗶𝗡𝗝𝗔  يعمـل .. بنجـاح ☑️ 𓆩 **\n"
    zed_caption += f"**{Z_EMOJI} إصـدار التـيليثون :** `{version.__version__}\n`"
    zed_caption += f"**{Z_EMOJI} إصـدار نينجا :** `{zedversion}`\n"
